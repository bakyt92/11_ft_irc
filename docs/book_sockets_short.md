...
* возвращает дескриптор сокета, который вы можете позже использовать в системных вызовах (-1 в случае ошибки, errno = код ошибки)   
...
* в случае ошибки вернёт -1 и установит переменную errno
* не вызываем bind(), нас заботит не наш локальный порт, а удалённый, ядро подберёт локальный порт и подключённый сайт будет получать от нас информацию
