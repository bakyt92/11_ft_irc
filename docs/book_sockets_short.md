...
* **брандмауэр** скрывает сеть от остального мира своей собственной защитой
...
* начинающему не нужно беспокоиться об NAT, он для вас прозрачен
* Например:
...
* наиболее частые номера частных сетей: `10.x.x.x` и `192.168.x.x`, менее распространены `172.y.x.x`, y лежит между 16 и 31
...
* IPv6 тоже имеет частные сети
  + начинаются с `fdxx:`
  + NAT и IPv6 как правило не смешиваются, однако в теории у вас будет столько адресов, что NAT больше не
понадобится
  + можно выделить для себя адреса в сети, которая не будет доступна снаружи:
